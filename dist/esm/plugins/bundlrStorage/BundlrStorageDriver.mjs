...
class BundlrStorageDriver {
  ...
  async initBundlr() {
    ...
    const identity = (_this$_options$identi = this._options.identity) !== null && _this$_options$identi !== void 0 ? _this$_options$identi : this._metaplex.identity(); // const bundlr = isKeypairSigner(identity)
    //   ? this.initNodeBundlr(address, currency, identity, options)
    //   : await this.initWebBundlr(address, currency, identity, options);

    const bundlr = await this.initWebBundlr(address, currency, identity, options);
